...
:: Get HWID using fallback method
set "HWID="
for /f "tokens=2 delims==" %%A in ('"wmic diskdrive get SerialNumber /value 2>nul | findstr /r /c:"SerialNumber=""') do (
    set "HWID=%%A"
)
if not defined HWID (
...
:: Valid license keys (line-by-line comparison)
setlocal EnableDelayedExpansion
...
set "STORED_KEY=" & set "STORED_HWID="
...
set "FOUND=0"
for /f %%K in ('type "%VALID_KEYS_FILE%"') do (
    if /i "%%K"=="%KEY%" set FOUND=1
)
